...
// Données des produits avec Système de Promotion (Ancien prix vs Nouveau prix)
const products = ref([
  { id: 1, name: 'ENFANT RICHES DEPRIMES', category: 'VINTAGE', price: '20000', oldPrice: '25000', rating: '5.0 (89)', image: '/images/IMG_2500.png', tag: 'OFFRE LIMITÉE' },
  { id: 2, name: 'Roughplay NYPD Edition', category: 'STREETWEAR', price: '20000', rating: '4.9 (120)', image: '/images/IMG_2724.png', tag: 'Nouveau' },
  { id: 3, name: 'ENFANT RICHES DEPRIMES', category: 'VINTAGE', price: '20000', oldPrice: '25000', rating: '4.7 (75)', image: '/images/IMG_2868 (1).png', tag: 'Top Ventes' },
  { id: 4, name: 'West Coast Choppers Custom', category: 'ART', price: '20000', rating: '4.9 (34)', image: '/images/IMG_2991.png', tag: 'Édition Limitée' },
  { id: 5, name: 'METALLICA', category: 'MINIMAL', price: '20000', rating: '4.6 (52)', image: '/images/IMG_3107.png', tag: 'Premium' },
  { id: 6, name: 'EMOTIONS', category: 'STREETWEAR', price: '20000', rating: '5.0 (108)', image: '/images/IMG_3205.png', tag: 'Must Have' },
...
// WhatsApp Config
const whatsappNumber = "2290155232950"; // Format sans le '+' pour l'URL

// Partager un produit seul
const shareProduct = () => {
  if (navigator.share) {
    navigator.share({
      title: 'Happy Cotton',
      text: `Regarde cette pépite : ${selectedProduct.value.name} est en promo à ${selectedProduct.value.price} FCFA !`,
...
// Envoyer tout le panier par WhatsApp
...
// Lien WhatsApp flottant (Produit sélectionné)
const whatsappLink = computed(() => {
  const msg = encodeURIComponent(`Bonjour Happy Cotton, je souhaite commander l'article en promotion : ${selectedProduct.value.name} (${selectedProduct.value.price} FCFA)`);
...
<template>
  <div class="max-w-md mx-auto bg-white min-h-screen shadow-2xl relative overflow-x-hidden font-sans text-zinc-900">
    ...
    <section class="p-4">
      <div class="bg-[#1a1a1a] rounded-[2.5rem] p-8 text-white relative overflow-hidden shadow-xl">
        <div class="absolute top-0 right-0 p-4">
            <span class="bg-red-600 text-[10px] font-black px-3 py-1 rounded-full animate-pulse uppercase">-20% MAINTENANT</span>
        </div>
        <h2 class="text-3xl font-bold leading-tight mb-2 italic">Collection <br> Premium 2026</h2>
        <p class="text-gray-400 text-xs mb-6">Des t-shirts d'exception à prix réduit.</p>
      ...
      <div class="grid grid-cols-2 gap-4 mb-10">
        <div v-for="item in products" :key="item.id" @click="selectedProduct = item" class="group cursor-pointer">
          <div class="relative aspect-[3/4] bg-gray-100 rounded-[2rem] overflow-hidden mb-3 border border-transparent transition-all" :class="selectedProduct.id === item.id ? 'border-zinc-900 ring-1 ring-zinc-900' : ''">
            ...
            <div v-if="item.oldPrice" class="absolute top-3 left-3 bg-red-600 text-white text-[7px] font-black px-2 py-1 rounded shadow-sm uppercase italic animate-pulse">PROMO</div>
            <div v-else-if="item.tag" class="absolute top-3 left-3 bg-white text-[7px] font-black px-2 py-1 rounded shadow-sm uppercase italic">{{ item.tag }}</div>
          </div>
          <h4 class="font-bold text-[11px] truncate uppercase tracking-tighter">{{ item.name }}</h4>
          <div class="flex items-center gap-2 mt-0.5">
             <span class="font-black text-[12px] text-red-600">{{ item.price }} FCFA</span>
             <span v-if="item.oldPrice" class="text-[9px] text-zinc-400 line-through">{{ item.oldPrice }}F</span>
          </div>
        </div>
      </div>
    </main>
      ...
      <div class="flex justify-between items-start mb-4">
        <div>
            ...
            <div class="flex items-center gap-2 mt-1">
                <span class="text-sm font-black text-red-600">{{ selectedProduct.price }} FCFA</span>
                <span v-if="selectedProduct.oldPrice" class="text-xs text-zinc-400 line-through italic">{{ selectedProduct.oldPrice }} FCFA</span>
            </div>
        </div>
    ...
    <footer class="bg-zinc-900 text-white rounded-t-[3rem] p-10 mt-20 text-center">
      ...
      <p class="text-[7px] text-zinc-600 uppercase tracking-widest">© 2026 Happy Cotton - Tous droits réservés</p>
    </footer>
          ...
          <div class="flex-1 overflow-y-auto space-y-4">
            ...
            <div v-for="item in cartItems" :key="item.cartId" class="flex gap-4 bg-zinc-50 p-3 rounded-2xl">
              ...
              <div class="flex-1">
                ...
                <p class="text-xs font-black mt-1 text-red-600">{{ item.price }} FCFA ({{ item.chosenSize }})</p>
              </div>
